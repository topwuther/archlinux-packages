...
contents=$(cat "$1/license.dat" | sed "s/XXXXXXXXXXXX/000000000000/g")
echo "$contents">"$1/license.dat"

#Get file offset
echo "file $1/quartus/linux/libsys_cpt.so
...
for i in {0..2} ; do
      ...
      ./ehex -name "$1/quartus/linux/libsys_cpt.so" -data "${data[$i]}" --offset $data_offset
done
